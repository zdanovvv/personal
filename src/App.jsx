...
// --- DRAG HANDLE ---
const DragHandle = () => (
  <div className="absolute top-3 left-3 z-20 p-2 bg-black/60 backdrop-blur-sm rounded-lg cursor-grab active:cursor-grabbing text-white hover:scale-110 transition-transform">
...
export default function App() {
  ...
  const getSortedData = (data, sortType) => {
    if (sortType === 'manual') return data;
    const sorted = [...data];
    ...
    if (sortType === 'newest') return sorted.sort((a, b) => b.id - a.id);
            ...
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl w-full max-w-lg border border-slate-200 dark:border-zinc-700 shadow-2xl my-auto">
              ...
              <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-2">
                {editingItem.type === 'socials' ? (
                  <>
                     <div><label className="text-xs font-bold uppercase text-slate-400">Platform Icon</label><select className="w-full p-2 rounded bg-slate-100 dark:bg-zinc-800 outline-none" value={editingItem.icon} onChange={e => setEditingItem({...editingItem, icon: e.target.value})}><option value="email">Gmail / Email</option><option value="github">Github</option><option value="instagram">Instagram</option><option value="linkedin">LinkedIn</option><option value="discord">Discord</option><option value="twitter">Twitter / X</option><option value="website">Website / Link</option></select></div>
                  ...
                  <motion.div key="projects" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                    ...
                    {sortConfig.projects === 'manual' ? (
                       <Reorder.Group 
                          ...
                          className={editMode ? "flex flex-col gap-6 max-w-2xl mx-auto" : "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6"}
                       >
                          {projects.map((p) => (
                            <Reorder.Item 
                              key={p.id} 
                              ...
                              whileDrag={{ scale: 1.02, boxShadow: "0px 10px 30px rgba(0,0,0,0.3)", zIndex: 50, cursor: "grabbing" }}
                              layout
                              transition={{ type: "spring", stiffness: 300, damping: 30 }}
                              className={`relative overflow-hidden rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 group ${editMode ? 'flex gap-4 items-center p-2' : ''}`}
                            >
                               {editMode && <DragHandle />}
                  ...
                  <motion.div key="games" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                    ...
                    {sortConfig.games === 'manual' ? (
                      <Reorder.Group 
                        ...
                        values={games} 
                        onReorder={setGames} 
                        className={editMode ? "flex flex-col gap-4 max-w-2xl mx-auto" : "grid grid-cols-2 md:grid-cols-4 gap-4"}
                      >
                        {games.map((g) => (
                          <Reorder.Item 
                            key={g.id} 
                            value={g} 
                            dragListener={editMode} 
                            whileDrag={{ scale: 1.05, boxShadow: "0px 10px 25px rgba(0,0,0,0.5)", zIndex: 50, cursor: "grabbing" }}
                            layout
                            transition={{ type: "spring", stiffness: 300, damping: 25 }}
                            className={`group relative rounded-2xl bg-slate-200 dark:bg-zinc-900 overflow-hidden border border-slate-200 dark:border-zinc-800 shadow-lg ${editMode ? 'flex items-center gap-4 p-2 h-24' : 'aspect-[3/4]'}`}
                          >
                             {editMode && <DragHandle />}
                           ...
                           >
                              {stories.map((s) => (
                                <Reorder.Item 
                                  ...
                                  value={s} 
                                  dragListener={editMode} 
                                  whileDrag={{ scale: 1.02, boxShadow: "0px 5px 15px rgba(0,0,0,0.2)", zIndex: 50, cursor: "grabbing" }}
                                  layout
                                  transition={{ type: "spring", stiffness: 300, damping: 30 }}
                                  className="relative flex flex-col md:flex-row gap-6 p-6 rounded-3xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 hover:border-indigo-500 transition-all group cursor-grab active:cursor-grabbing"
                                >
                                   {editMode && <DragHandle />}
...
// --- SUB-COMPONENTS (Modified for List View Compatibility) ---

const ProjectCardContent = ({ p, editMode, openEditModal, initiateCrop, setProjects, projects }) => (
  <>
    <div className={`${editMode ? 'w-24 h-24 rounded-xl' : 'h-48'} bg-slate-200 dark:bg-zinc-800 relative shrink-0 overflow-hidden`}>
        ...
        {editMode && <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10"><label className="cursor-pointer bg-white text-black px-2 py-1 rounded text-[10px] font-bold hover:scale-105"><span>📷</span><input type="file" className="hidden" onChange={(e) => initiateCrop(e, 'projects', p.id)}/></label></div>}
    </div>
    <div className={editMode ? 'flex-1 p-2' : 'p-6'}>
      <h3 className="font-bold text-xl mb-1">{p.title}</h3>
      <p className="text-xs font-mono text-indigo-500 mb-2">{p.tech}</p>
      {!editMode && <p className="text-sm text-slate-500 dark:text-zinc-400 line-clamp-3">{p.desc}</p>}
      {editMode && <button onClick={() => openEditModal(p, 'projects')} className="text-xs bg-indigo-100 dark:bg-zinc-800 px-3 py-1 rounded font-bold text-indigo-600">Edit</button>}
    </div>
    {editMode && <button onClick={() => setProjects(projects.filter(x => x.id !== p.id))} className="mr-4 text-red-500 font-bold hover:scale-110">✕</button>}
  </>
);

const GameCardContent = ({ g, editMode, openEditModal, initiateCrop, setGames, games }) => (
  <>
     <div className={`w-full h-full transition-transform duration-500 ${!editMode && 'group-hover:scale-110'}`}>
     ...
     {!editMode && <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent opacity-80 transition-opacity duration-300" />}
     
     {/* Content Layout changes based on Edit Mode */}
     <div className={`absolute inset-0 p-4 flex ${editMode ? 'flex-row items-center gap-4 bg-black/40' : 'flex-col justify-end'}`}>
        <div className={editMode ? 'flex-1 pl-12' : ''}>
           ...
           <div className="flex items-center gap-2 mb-2">
              ...
              {!editMode && <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider">{g.status}</span>}
           </div>
           {!editMode && (
             <div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-[grid-template-rows] duration-300 ease-out">
                 <div className="overflow-hidden">
                     <p className="text-xs text-zinc-300 mt-2 leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">{g.desc || "Tidak ada deskripsi tambahan."}</p>
                 </div>
             </div>
           )}
        </div>
        ...
        {editMode && (
           <div className="flex flex-col gap-2 z-20">
              <label className="cursor-pointer bg-white text-black px-3 py-1 rounded-full font-bold text-[10px] hover:scale-105 text-center"><span>📷</span><input type="file" className="hidden" onChange={(e) => initiateCrop(e, 'games', g.id)}/></label>
              ...
              <button onClick={(e) => {e.preventDefault(); setGames(games.filter(x => x.id !== g.id))}} className="bg-red-600 text-white px-3 py-1 rounded-full font-bold text-[10px] hover:bg-red-700">Del</button>
           </div>
        )}
     </div>
  </>
...
const StoryCardContent = ({ s, editMode, openEditModal, initiateCrop, setStories, stories, setReadingStory }) => (
  <>
    <div className={`shrink-0 relative overflow-hidden bg-slate-200 dark:bg-zinc-800 ${editMode ? 'w-24 h-24 rounded-xl' : 'w-full md:w-48 aspect-[2/3] rounded-xl'}`}>
      ...
      {editMode && <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10"><label className="cursor-pointer bg-white text-black px-2 py-1 rounded text-[10px] font-bold hover:scale-105"><span>📷</span><input type="file" className="hidden" onChange={(e) => initiateCrop(e, 'stories', s.id)}/></label></div>}
    </div>
    <div className="flex-1">
      <div className="flex justify-between items-start">
        <div><h2 className={`font-bold group-hover:text-indigo-500 transition-colors ${editMode ? 'text-lg' : 'text-2xl mb-2'}`}>{s.title}</h2><span className="px-3 py-1 rounded-full bg-slate-100 dark:bg-zinc-800 text-xs font-mono">{s.date}</span></div>
        {editMode && <button onClick={() => setStories(stories.filter(x => x.id !== s.id))} className="text-red-500 hover:text-red-600 bg-red-50 dark:bg-red-500/10 px-3 py-1 rounded-lg text-sm font-bold transition-colors">Del 🗑️</button>}
      </div>
      {!editMode && <p className="mt-4 text-slate-600 dark:text-zinc-400 leading-relaxed line-clamp-3">{s.snippet}</p>}
      <div className="flex items-center gap-4 mt-6">
        {!editMode && <button onClick={() => setReadingStory(s)} className="text-indigo-500 font-bold text-sm hover:underline flex items-center gap-1">Read Full Story <span>→</span></button>}
